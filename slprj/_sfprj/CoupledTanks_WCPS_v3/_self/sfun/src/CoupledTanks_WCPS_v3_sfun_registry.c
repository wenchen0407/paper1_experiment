...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_CoupledTanks_WCPS_v3_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_CoupledTanks_WCPS_v3_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_CoupledTanks_WCPS_v3_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "CoupledTanks_WCPS_v3") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_CoupledTanks_WCPS_v3_autoinheritance_info(nlhs,plhs,2,newRhs);
  }
...
{
  extern unsigned int sf_CoupledTanks_WCPS_v3_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "CoupledTanks_WCPS_v3") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_CoupledTanks_WCPS_v3_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_CoupledTanks_WCPS_v3_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"CoupledTanks_WCPS_v3")) {
    return(sf_CoupledTanks_WCPS_v3_method_dispatcher(simstructPtr,
...
extern void CoupledTanks_WCPS_v3_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    CoupledTanks_WCPS_v3_terminator();
    sf_debug_terminate();
  }
...
extern void CoupledTanks_WCPS_v3_initializer(void);
extern void CoupledTanks_WCPS_v3_register_exported_symbols(SimStruct* S);
extern void CoupledTanks_WCPS_v3_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  CoupledTanks_WCPS_v3_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      CoupledTanks_WCPS_v3_debug_initialize();
    }

    CoupledTanks_WCPS_v3_initializer();
  }
