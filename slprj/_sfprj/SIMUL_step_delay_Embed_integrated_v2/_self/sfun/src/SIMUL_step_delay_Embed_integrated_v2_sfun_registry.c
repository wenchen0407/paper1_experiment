...
{
  extern unsigned int
    sf_SIMUL_step_delay_Embed_integrated_v2_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_SIMUL_step_delay_Embed_integrated_v2_process_check_sum_call(nlhs,plhs,
       nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_SIMUL_step_delay_Embed_integrated_v2_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_SIMUL_step_delay_Embed_integrated_v2_autoinheritance_info(nlhs,plhs,
       nrhs,prhs))
    return 1;
  return 0;
}
  ...
  plhs[], int nrhs, const mxArray * prhs[] )
{
  ...
  if (strcmp(machineName, "SIMUL_step_delay_Embed_integrated_v2") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return
      sf_SIMUL_step_delay_Embed_integrated_v2_get_eml_resolved_functions_info
      (nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_SIMUL_step_delay_Embed_integrated_v2_method_dispatcher
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"SIMUL_step_delay_Embed_integrated_v2")) {
    return(sf_SIMUL_step_delay_Embed_integrated_v2_method_dispatcher
...
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    SIMUL_step_delay_Embed_integrated_v2_terminator();
    sf_debug_terminate();
  }
...
extern void SIMUL_step_delay_Embed_integrated_v2_register_exported_symbols
  (SimStruct* S);
extern void SIMUL_step_delay_Embed_integrated_v2_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  SIMUL_step_delay_Embed_integrated_v2_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      SIMUL_step_delay_Embed_integrated_v2_debug_initialize();
    }

    SIMUL_step_delay_Embed_integrated_v2_initializer();
  }
