...
{
  extern unsigned int
    sf_SIMUL_step_delay_Embed_integrated_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_SIMUL_step_delay_Embed_integrated_process_check_sum_call(nlhs,plhs,nrhs,
       prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_SIMUL_step_delay_Embed_integrated_autoinheritance_info(
    int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_SIMUL_step_delay_Embed_integrated_autoinheritance_info(nlhs,plhs,nrhs,
       prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_SIMUL_step_delay_Embed_integrated_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "SIMUL_step_delay_Embed_integrated") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_SIMUL_step_delay_Embed_integrated_get_eml_resolved_functions_info
      (nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_SIMUL_step_delay_Embed_integrated_method_dispatcher
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"SIMUL_step_delay_Embed_integrated")) {
    return(sf_SIMUL_step_delay_Embed_integrated_method_dispatcher(simstructPtr,
...
extern void SIMUL_step_delay_Embed_integrated_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    SIMUL_step_delay_Embed_integrated_terminator();
    sf_debug_terminate();
  }
...
extern void SIMUL_step_delay_Embed_integrated_initializer(void);
extern void SIMUL_step_delay_Embed_integrated_register_exported_symbols
  (SimStruct* S);
extern void SIMUL_step_delay_Embed_integrated_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  SIMUL_step_delay_Embed_integrated_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      SIMUL_step_delay_Embed_integrated_debug_initialize();
    }

    SIMUL_step_delay_Embed_integrated_initializer();
  }
